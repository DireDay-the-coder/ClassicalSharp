...
namespace ClassicalSharp.Commands
{
    ...
    public sealed class HelpCommand : Command
    {
        ...
        public override void ExecuteHacks(string args)
        {
            try
            {
                ...
                else if (args.ToLower().StartsWith(".help"))
                {
                    ...
                    game.Chat.Add("&9.reach <blocks> - Increases Reach");
                    game.Chat.Add("&9.hacks <on/off> - Bypasses -hax on server map motd's");
    ...
    public sealed class CuboidCommand : Command
    {
        ...
        bool ParseBlock(string[] args)
        {
            ...
            if (Utils.CaselessEq(args[1], "yes"))
            {
                persist = true;
                return true;
            }
            ...
            if (raw == -1)
            {
                game.Chat.Add("&eCuboid: &c\"" + args[1] + "\" is not a valid block name or id.");
                return false;
            }
            ...
            {
                game.Chat.Add("&eCuboid: &cThere is no block with id \"" + args[1] + "\".");
                return false;
            }
